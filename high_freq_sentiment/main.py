...
from src.z_scoring import (
    RollingZScorer, ExponentialZScorer, CrossSectionalZScorer,
    VectorizedZScoreLayer, ZScoreConfig
)
from src.echo_chamber import (
    MarketSentimentOrthogonalizer, PCAOrthogonalizer,
    EchoChamberEliminator, OrthogonalizationConfig
)
from src.signal_generation import (
    SignalGenerator, SignalConfig, TradingSignal, SignalType,
    SignalEvaluator
)
from src.sentiment_extraction import (
    SentimentExtractionLayer, SentimentTokenizer, create_mock_sentiment_layer
)
...
class SentimentArbitrageSystem:
    ...
    def __init__(self, config: Optional[SystemConfig] = None):
        ...
        print(f"Initializing Vectorized Sentiment Arbitrage System...")
        ...
        if self.config.use_kalman_ensemble:
            self.kalman = VectorizedKalmanFilterBank(
                ...
                n_filters=self.config.n_kalman_filters
            )
        else:
        ...
        if self.config.z_score_method == 'layer':
            ...
            self.z_scorer = ExponentialZScorer(
                n_stocks=self.config.n_stocks,
                config=z_config
            )
        else:  # rolling
        ...
        echo_config = OrthogonalizationConfig(
            method=self.config.orthogonalization_method,
        ...
        if self.config.use_triton:
            ...
            self.sentiment_layer = create_mock_sentiment_layer(
                n_stocks=self.config.n_stocks
            )
    ...
    def update(
        ...
        sector_sentiments: Optional[np.ndarray] = None,
        timestamp: int = 0
    ) -> Dict:
        """
        ...
        idio_sentiment = self.eliminator.eliminate_echo_chamber(
            ...
            sector_sentiments=cp.array(sector_sentiments) if sector_sentiments is not None else None
        )
    ...
    def run_backtest(
        ...
        for t in range(n_timesteps):
            ...
            if sentiment_data is not None:
                ...
                if price_changes.shape[0] > 0:
                    sentiment_scores = price_changes[-1, :] * 0.5 + np.random.randn(self.config.n_stocks) * 0.3
                else:
                    sentiment_scores = np.random.randn(self.config.n_stocks) * 0.5
            ...
            for signal in results['signals']:
                quantity, cost = self.signal_generator.execute_signal(
                    signal,
                    portfolio_value
                )
            ...
            # Close positions
            for stock_id in results['n_stocks_to_close'] if isinstance(results['n_stocks_to_close'], list) else []:
                pnl, value = self.signal_generator.close_position(
                    stock_id,
                    float(price_data[t, stock_id])
                )
        ...
        backtest_results = {
            ...
            'annualized_return': ((portfolio_value / initial_portfolio_value) ** (252 / n_timesteps) - 1) if n_timesteps > 0 else 0,
...
def main():
    ...
    with open(results_file, 'w') as f:
        ...
        serializable_results = {
            ...
            if k not in ['portfolio_values', 'all_results']
        }
