...
class StockDataIngestion:
    ...
    def _init_database(self):
        ...
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_fundamentals_ts_code ON fundamentals(ts_code)"
        )
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_fundamentals_end_date ON fundamentals(end_date)"
        )
    ...
    def _fetch_stock_history_akshare(
        self, ts_code: str, start_date: str, end_date: str, period: str = "daily"
    ) -> pd.DataFrame:
        """
    ...
    def _fetch_stock_history_baostock(
        self, ts_code: str, start_date: str, end_date: str, frequency: str = "d"
    ) -> pd.DataFrame:
        """
    ...
    def fetch_stock_history(
        self, ts_code: str, start_date: str, end_date: str, use_cache: bool = True
    ) -> pd.DataFrame:
        """
    ...
    def batch_fetch_stock_history(
        self, ts_codes: list[str], start_date: str, end_date: str, batch_size: int | None = None
        ...
        with ThreadPoolExecutor(max_workers=10) as executor:
            futures = {
                executor.submit(self.fetch_stock_history, ts_code, start_date, end_date): ts_code
    ...
    def _fetch_fundamentals_akshare(
        ...
        try:
            ...
            df = ak.stock_individual_fund_indicator(symbol=ts_code.split(".")[0])
    ...
    def _fetch_fundamentals_baostock(self, ts_code: str, year: int, quarter: int) -> pd.DataFrame:
        """
        ...
        try:
            ...
            rs = bs.query_performance_data(code, year, quarter)
    ...
    def _validate_ohlcv_data(self, df: pd.DataFrame, ts_code: str) -> pd.DataFrame:
        ...
        if invalid_count > 0:
            logger.warning(
                f"{ts_code}: Found {invalid_count} rows with invalid OHLC relationships, removing"
            )
            df = df[valid_rows]
        ...
        for col in ["close", "volume"]:
            if col in df.columns and len(df) > self.config.min_data_points:
                ...
                if std > 0:
                    ...
                    if outlier_count > 0:
                        logger.warning(
                            f"{ts_code}: Found {outlier_count} outliers in {col}, applying winsorization"
                        )
                        df.loc[outlier_mask, col] = (
                            mean
                            + std
                            * self.config.outlier_std_threshold
                            * np.sign(df.loc[outlier_mask, col] - mean)
                        )

        # Check for missing data ratio
        if len(df) < self.config.min_data_points:
            logger.warning(
                f"{ts_code}: Insufficient data points ({len(df)} < {self.config.min_data_points})"
            )
        ...
        if removed_count > 0:
            logger.info(
                f"{ts_code}: Cleaned {removed_count} rows ({removed_count / original_count:.1%} removal rate)"
            )
    ...
    def get_stock_data_from_db(self, ts_code: str, start_date: str, end_date: str) -> pd.DataFrame:
        """
...
def main():
    ...
    df = ingestion.fetch_stock_history("000001.sz", "20230101", "20231231")
    print(df.head())
