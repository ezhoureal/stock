...
class SquareRootKalmanFilter:
    ...
    def _estimate_adaptive_noise(self, innovation: cp.ndarray, innovation_cov: cp.ndarray):
        """
    ...
    def _compute_log_likelihood(self, innovation: cp.ndarray, innovation_cov: cp.ndarray) -> float:
        """
    ...
    def filter_batch(self, measurements_batch: cp.ndarray) -> tuple[cp.ndarray, cp.ndarray]:
        """
        ...
        for t in range(n_timesteps):
            ...
            # Update
            beta_filtered[t, :] = self.update_step(measurements_batch[t, :], beta_pred, P_pred_sqrt)

        return beta_filtered

    def get_confidence_intervals(self, confidence: float = 0.95) -> tuple[cp.ndarray, cp.ndarray]:
        """
    ...
    def __init__(
        ...
        if filter_configs:
            for config in filter_configs:
                filter_bank = [
                    SquareRootKalmanFilter(n_stocks=n_stocks, **config) for _ in range(n_filters)
                ]
            ...
            for i in range(n_filters):
                ...
                self.filters.append(SquareRootKalmanFilter(n_stocks=n_stocks, **config))

    def update_ensemble(self, measurements: cp.ndarray) -> cp.ndarray:
        """
        ...
        for i, kalman_filter in enumerate(self.filters):
            ...
            # Update
            ensemble_states[i, :] = kalman_filter.update_step(measurements, beta_pred, P_pred_sqrt)

        # Return ensemble mean
        return cp.mean(ensemble_states, axis=0)

    def get_ensemble_statistics(self, measurements: cp.ndarray) -> dict:
        """
