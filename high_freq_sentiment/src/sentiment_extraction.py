...
class SentimentTokenizer:
    ...
    def __init__(self, max_length: int = 512, model_name: str = "distilbert-base-uncased"):
        ...
        try:
            ...
            self.tokenizer = AutoTokenizer.from_pretrained("ProsusAI/finbert", use_fast=True)
    ...
    def tokenize_batch(self, texts: list[str], return_tensors: str = "np") -> dict[str, np.ndarray]:
        """
...
class TritonSentimentClient:
    ...
    def __init__(
        ...
        model_config: TritonModelConfig | None = None,
        verbose: bool = False,
    ):
        """
        ...
        self.client = grpc_client.InferenceServerClient(url=server_url, verbose=verbose)
    ...
    def _check_model_ready(self):
        ...
        try:
            if not self.client.is_model_ready(
                self.model_config.model_name, self.model_config.model_version
            ):
    ...
    async def infer_batch_async(
        ...
        # Create output
        outputs = [grpc_client.InferRequestedOutput("logits")]
    ...
    def infer_batch(self, input_ids: np.ndarray, attention_mask: np.ndarray) -> np.ndarray:
        """
    ...
    def infer_large_batch(
        self, texts: list[str], tokenizer: "SentimentTokenizer", batch_size: int | None = None
    ) -> np.ndarray:
        """
        ...
        for i in range(0, n_texts, batch_size):
            ...
            # Inference
            logits = self.infer_batch(encoded["input_ids"], encoded["attention_mask"])
...
class SentimentExtractionLayer:
    ...
    def __init__(
        self,
        triton_client: TritonSentimentClient | None = None,
        tokenizer: SentimentTokenizer | None = None,
        n_classes: int = 3,
    ):
        """
        ...
        self.sentiment_mapping = {"negative": -1.0, "neutral": 0.0, "positive": 1.0}
    ...
    def extract_sentiment_batch(
        self, texts: list[str], return_confidence: bool = True, return_logits: bool = False
    ) -> SentimentOutput:
        """
        ...
        if self.triton_client:
            logits = self.triton_client.infer_batch(encoded["input_ids"], encoded["attention_mask"])
        else:
    ...
    def extract_market_sentiment(self, texts: list[str]) -> float:
        """
    ...
    def get_statistics(self) -> dict:
        ...
        return {
            ...
            "avg_tokens_per_request": self.total_tokens / self.n_requests
...
def test_sentiment_extraction():
    ...
    output = sentiment_layer.extract_sentiment_batch(
        sample_texts, return_confidence=True, return_logits=True
    )

    # Print results
    print("\nSample Texts and Sentiment Scores:")
    for i, (text, score, confidence, label) in enumerate(
        zip(sample_texts, output.sentiment_scores, output.confidence_scores, output.class_labels)
    ):
