...
class VectorizedArbitrageEngine:
    ...
    def remove_echo_chamber(
        self, stock_sentiments: cp.ndarray, market_sentiment: cp.ndarray
    ) -> cp.ndarray:
        """
        ...
        idio_sentiment = (
            idio_sentiment - cp.mean(idio_sentiment, axis=0, keepdims=True) + market_mean
        )
...
class PerformanceMonitor:
    ...
    def calculate_metrics(self) -> dict:
        ...
        return {
            ...
            "total_trades": len(
                [s for s in self.signals_history if s["buy_count"] + s["sell_count"] > 0]
            ),
        }
