...
class RollingZScorer:
    ...
    def __init__(self, n_stocks: int = 500, config: ZScoreConfig | None = None):
        """
    ...
    def _compute_z_scores(self, window_data: cp.ndarray, current_value: cp.ndarray) -> cp.ndarray:
        """
...
class VectorizedZScoreLayer:
    ...
    def get_combined_z_score(self, z_scores_dict: dict, weights: dict | None = None) -> cp.ndarray:
        """
...
def benchmark_z_scores(n_stocks: int = 500, n_updates: int = 1000):
    ...
    for method_name, scorer in methods.items():
        ...
        print(
            f"{method_name:20s}: {avg_time:.4f} ms per update ({n_updates / elapsed:.0f} updates/sec)"
        )
