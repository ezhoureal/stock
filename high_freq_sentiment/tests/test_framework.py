...
class TestFramework:
    ...
    def _test_kalman_filter_ensemble(self) -> Dict:
        ...
        return {
            'n_filters': n_filters,
            'ensemble_std_mean': float(cp.mean(stats['std']))
        }
    ...
    def _test_z_score_rolling(self) -> Dict:
        ...
        assert abs(np.std(valid_scores) - 1.0) < 0.2, f"Std should be ~1, got {np.std(valid_scores)}"
    ...
    def _test_projection_orthogonalization(self) -> Dict:
        ...
        idio_result = orthogonalizer.orthogonalize_projection(
            cp.array(stock_sentiment),
            cp.array([market_sentiment])
        ).get()
    ...
    def _test_echo_chamber_combination(self) -> Dict:
        ...
        idio_sentiment = eliminator.eliminate_echo_chamber(
            stock_sentiment,
            market_sentiment=0.5
        )
        
        # Verify orthogonality
        correlation = eliminator.verify_orthogonality(
            idio_sentiment,
            cp.array([0.5])
        )
        ...
        return {
            'correlation': float(correlation),
            'mean_idio': float(cp.mean(idio_sentiment))
        }
    ...
    def _test_signal_buy_conditions(self) -> Dict:
        ...
        config = SignalConfig(
            ...
            long_price_threshold=-0.5,
            long_dislocation_threshold=2.0
        )
    ...
    def _test_signal_exit_conditions(self) -> Dict:
        ...
        return {
            'closed_positions': len(stocks_to_close)
        }
    ...
    def _test_signal_position_sizing(self) -> Dict:
        ...
        config = SignalConfig(
            ...
            position_sizing_method="signal_strength"
        )
        ...
        return {
            'position_size': signals[0].position_size if signals else 0
        }
    ...
    def _test_full_pipeline(self) -> Dict:
        ...
        for t in range(20, n_timesteps):
            # Normalize
            z_sentiment = (sentiment[t, :] - np.mean(sentiment[t-20:t, :], axis=0)) / np.std(sentiment[t-20:t, :], axis=0)
            z_prices = (prices[t, :] - np.mean(prices[t-20:t, :], axis=0)) / np.std(prices[t-20:t, :], axis=0)
    ...
    def _print_summary(self):
        ...
        with open('/home/zireael/.openclaw/workspace/sentiment_arbitrage/tests/test_results.json', 'w') as f:
