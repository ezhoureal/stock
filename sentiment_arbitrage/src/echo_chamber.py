...
class MarketSentimentOrthogonalizer:
    ...
    def __init__(
        self,
        n_stocks: int = 500,
        config: Optional[OrthogonalizationConfig] = None
    ):
        """
    ...
    def orthogonalize_projection(
        ...
        sector_sentiments: Optional[cp.ndarray] = None
    ) -> cp.ndarray:
        """
...
class PCAOrthogonalizer:
    ...
    def fit(self, sentiment_data: cp.ndarray):
        ...
        print(f"Explained variance by top {self.n_components} PCs: {cp.sum(explained_var).get():.3f}")
...
class EchoChamberEliminator:
    ...
    def __init__(
        ...
        if 'projection' in methods:
            self.orthogonalizers['projection'] = MarketSentimentOrthogonalizer(
                n_stocks, config
            )
    ...
    def eliminate_echo_chamber(
        ...
        sector_sentiments: Optional[cp.ndarray] = None,
        factors: Optional[cp.ndarray] = None
    ) -> cp.ndarray:
        """
...
def test_echo_chamber_elimination():
    ...
    idio_projection = projector.orthogonalize_projection(
        stock_sentiment_gpu[:, 0],
        market_sentiment_gpu[0]
    )
    
    correlation = projector.verify_orthogonality(
        idio_projection,
        cp.array([market_sentiment_gpu[0]])
    )
    ...
    idio_combined = eliminator.eliminate_echo_chamber(
        stock_sentiment_gpu[:, 0],
        market_sentiment=float(market_sentiment_gpu[0])
    )
