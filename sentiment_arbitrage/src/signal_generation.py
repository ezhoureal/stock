...
class SignalGenerator:
    ...
    def generate_signals(
        ...
        confidence_scores: Optional[cp.ndarray] = None,
        timestamp: int = 0
    ...
    def _check_exit_conditions(
        ...
        for stock_id, signal in self.active_positions.items():
            ...
            if self.config.exit_time_stop_hours > 0:
                ...
                if hours_held >= self.config.exit_time_stop_hours * 3600:  # Convert hours to seconds
                    should_exit = True
...
def test_signal_generation():
    ...
    for signal in signals:
        print(f"  Stock {signal.stock_id}: {signal.signal_type.name} | "
