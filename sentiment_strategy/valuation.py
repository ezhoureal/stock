...
class ValuationCalculator:
    ...
    def calculate_relative_metrics(
        ...
        For PE, PB, PEG: (company / sector) - 1
        For dividend: company - sector

        Returns:
            (relative_pe, relative_pb, dividend_diff, relative_peg)
        """
        # PE: Lower is better
        ...
        # PB: Lower is better
        if sector.pb_ratio > 0:
            relative_pb = (company.pb_ratio / sector.pb_ratio) - 1
        else:
            relative_pb = 0.0

        # Dividend: Higher is better (absolute difference)
        dividend_diff = company.dividend_yield - sector.dividend_yield

        # PEG: Lower is better
        if sector.peg_ratio and sector.peg_ratio > 0 and company.peg_ratio:
            relative_peg = (company.peg_ratio / sector.peg_ratio) - 1
        else:
            # If PEG not available, use PE trend as proxy
            relative_peg = 0.0
    ...
    def calculate_composite_score(
        ...
        dividend_score = dividend_diff * self.config.dividend_weight
    ...
    def calculate_intrinsic_value(
        ...
        """
        # Method 1: P/E based
        # If sector P/E is 20 and company P/E is 15, company is undervalued
        if sector.pe_ratio > 0:
        ...
        if methods:
            intrinsic_value = np.mean(methods)
